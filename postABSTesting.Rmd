...
connectivities %>%
  ...
  geom_line(col = connColor, size = 1)+
  theme_minimal()+
  geom_vline(aes(xintercept = burn.in+1), size = 0.5, lty = 2)+
  ylab("Connectivity")+
  xlab("Time slice")
...
connDF %>%
  ggplot(aes(x = slice, y = connectivity))+
  geom_line(aes(group = run), col = connColor, size = 0.2, alpha = 0.5)+
  theme_minimal()+
  geom_vline(aes(xintercept = burn.in+1), size = 0.7, lty = 2)+
  scale_color_viridis_c()+
  ylab("Connectivity")+
...
plotElements = list(geom_point(shape = 1),
             ...
             scale_color_manual(values = c(connColor, densColor, moduColor)),
