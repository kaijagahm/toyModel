...
nRuns <- 100
modelRuns50 <- vector(mode = "list", length = nRuns)
for(i in 1:nRuns){
  modelRuns50[[i]] <- runModel(N = 50, # Nodes in the network
                        edge.prob = edge.prob,
                        n.removed = n.removed,
                        burn.in = burn.in,
                        recovery = recovery,
...
modelRuns25 <- vector(mode = "list", length = nRuns)
for(i in 1:nRuns){
  modelRuns25[[i]] <- runModel(N = 25, # Nodes in the network
                        edge.prob = edge.prob,
                        n.removed = n.removed,
                        burn.in = burn.in,
                        recovery = recovery,
...
modelRuns75 <- vector(mode = "list", length = nRuns)
for(i in 1:nRuns){
  modelRuns75[[i]] <- runModel(N = 75, # Nodes in the network
                        edge.prob = edge.prob,
                        n.removed = n.removed,
                        burn.in = burn.in,
                        recovery = recovery,
...
modelRuns100 <- vector(mode = "list", length = nRuns)
for(i in 1:nRuns){
  modelRuns100[[i]] <- runModel(N = 100, # Nodes in the network
                        edge.prob = edge.prob,
                        n.removed = n.removed,
                        burn.in = burn.in,
                        recovery = recovery,
...
modelRunsList <- list(modelRuns25, modelRuns50, modelRuns75, modelRuns100)
```
...
dens <- map2(.x = modelRunsList, .y = ns, .f = function(.x, .y){
  densList <- lapply(.x, function(x){
    lapply(x, function(y){
      edge_density(y)
    }) %>%
      unlist() %>%
...
md <- map2(.x = modelRunsList, .y = ns, .f = function(.x, .y){
  densList <- lapply(.x, function(x){
    lapply(x, function(y){
      mean_distance(y)
    }) %>%
...
modu <- map2(.x = modelRunsList, .y = ns, .f = function(.x, .y){
  moduList <- lapply(.x, function(x){
...
networkLevel <- left_join(dens, md, by = c("run", "slice", "N")) %>%
  left_join(modu, by = c("run", "slice", "N"))

# NETWORK-LEVEL MEASURE DELTAS (before and after rewiring)
deltas <- networkLevel %>%
  filter(slice %in% c(burn.in, burn.in+1, burn.in+2)) %>%
  group_by(run) %>%
  summarize(dens1 = density[slice == burn.in+1]-density[slice == burn.in],
            dens2 = density[slice == burn.in+2]-density[slice == burn.in+1],
            md1 = meanDistance[slice == burn.in+1]-meanDistance[slice == burn.in],
            md2 = meanDistance[slice == burn.in+2]-meanDistance[slice == burn.in+1],
            modu1 = modularity[slice == burn.in+1]-modularity[slice == burn.in],
            modu2 = modularity[slice == burn.in+2]-modularity[slice == burn.in+1]) %>%
...
removedInfo <- lapply(modelRuns50, function(x){
  init <- V(x[[1]])$name
