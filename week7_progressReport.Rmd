...
runSim <- function(tmax = 10, # length of time over which to run the simulation
  ...
  for(day in 2:tmax){
    ...
    baseAdd <- 0.1 # p of gaining an edge given not connected in either of the previous two time steps
    newLose <- 0.3 # p of losing an edge given connected in prev time step but not prev prev
    lostAdd <- 0.3 # p of gaining an edge given connected in prev prev time step but not prev
    alwaysLose <- 0.1 # p of losing edge given connected in previous 2 time steps
    edges <- edges %>%
