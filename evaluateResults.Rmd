...
```{r}
realGraphs <- makeGraphs(edges = toUse, interval = "5 days", 
...
Run the model with `r nIndivs` individuals. Setting the burn.in to `length(realGraphs)` = `r length(realGraphs)` because for the purposes of testing the baseline dynamics, we don't actually care about the removal aspect.
...
for(i in 1:length(realGraphs)){
  p <- tidygraph::as_tbl_graph(realGraphs[[i]], nodes = coords$name) %>%
    ...
    theme(legend.position = "none")+
    ggtitle(paste("real networks: day", i))
  print(p)
}
```
...
```{r}
# How long does it take for the density to recover after the loss of a node?
reps <- vector(mode = "list", length = 100)
for(i in 1:length(reps)){
  graphs <- runModel(N = nIndivs, burn.in = length(realGraphs), doRemoval = TRUE, burn.out = length(realGraphs)) %>%
  lapply(., function(x){
    igraph::graph_from_adjacency_matrix(x, mode = "undirected")
  })
  reps[[i]] <- graphs
...
densities <- lapply(reps, function(x){
...
transitivities <- lapply(reps, function(x){
  lapply(x, igraph::transitivity) %>%
    setNames(., NULL) %>%
    unlist() %>%
    as.data.frame() %>%
    mutate(type = "model",
...
densities %>%
  ggplot(aes(x = timestep, y = density, col = factor(rep)))+
  geom_vline(xintercept = length(realGraphs))+
  geom_smooth(se = FALSE)+
  theme_minimal()+
  theme(legend.position = "none")

transitivities %>%
  ggplot(aes(x = timestep, y = transitivity, col = factor(rep)))+
  geom_vline(xintercept = length(realGraphs))+
  geom_smooth(se = FALSE)+
  theme_minimal()+
  theme(legend.position = "none")
